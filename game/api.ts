...
export const login = (tgWebAppData: string) =>
	Http.request.post('https://server.questioncube.xyz/auth').pipe(
...
export const buy = (accessToken: string, proposalId = 1) =>
	Http.request.post('https://server.questioncube.xyz/game/rest-proposal/buy').pipe(
...
export const collect = (accessToken: string) =>
	Http.request.post('https://server.questioncube.xyz/game/mined').pipe(
		Http.request.setHeader('User-Agent', UA),
		Http.request.setHeader('Content-Type', 'application/json'),
		Http.request.jsonBody({
			token: accessToken,
		...
		Effect.tap((r) => {
			if (r.status >= 400) {
				return r.text.pipe(Effect.map(console.error))
			}
		}),
...
export const statistics = Http.request
	.get('https://server.questioncube.xyz/chart')
	.pipe(Http.client.fetchOk, Effect.andThen(Http.response.schemaBodyJson(Stats)), Effect.scoped)
